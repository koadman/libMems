/*******************************************************************************
 * $Id: MemScorer.cpp,v 1.17 2004/03/01 02:40:08 darling Exp $
...
//	uint32 sample_size_min = 1000;
	...
	for(uint32 bucketI = 0; bucketI < table_size; bucketI++){
		set<MatchHashEntry*, MheCompare>::const_iterator mem_iter = mem_table[bucketI].begin();
		for(; mem_iter != mem_table[bucketI].end(); mem_iter++){
			for(uint32 seqI = 0; seqI < seq_count; seqI++){
				if((**mem_iter)[seqI] != (int64)NO_MATCH){
					for(uint32 seq2 = 0; seq2 < seq_count; seq2++){
						if((**mem_iter)[seq2] != (int64)NO_MATCH){
...
void MemScorer::AllocateMatrix(){
	...
	if(detail_list != NULL)
		delete[] detail_list;
	hit_matrix = new gnSeqI*[seq_count];
	for(uint32 k = 0; k < seq_count; k++){
		hit_matrix[k] = new gnSeqI[seq_count];
		for(uint32 m = 0; m < seq_count; m++)
			hit_matrix[k][m] = 0;
	}
	identity_matrix = new gnSeqI*[seq_count];
	for(uint32 l = 0; l < seq_count; l++){
		identity_matrix[l] = new gnSeqI[m_hit_resolution];
	...
	if(seq_count <= MAX_DETAIL_SEQS){
		...
		detail_list = new gnSeqI[list_size];
...
boolean MemScorer::HashMatch(IdmerList& match_list){
	
	IdmerList::iterator firstI = match_list.begin();
	IdmerList::iterator secondI;
