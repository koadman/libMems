...
/**
 * Test code to check that no two LCBs overlap each other
 */
void validateRangeIntersections( vector< MatchList >& lcb_list  ){
	cerr << "Re-implement validateRangeIntersections\n";
/*
	if( lcb_list.size() == 0 )
		return;
	// build LCB ranges
	uint lcbI = 0;
	uint seqI = 0;
	vector< LCB > lcb_bounds;
	uint seq_count = lcb_list[ 0 ].seq_table.size();
	for( ; lcbI < lcb_list.size(); lcbI++ ){
		LCB cur_lcb;
		for( seqI = 0; seqI < seq_count; seqI++ ){
			cur_lcb.start.push_back( NO_MATCH );
			cur_lcb.length.push_back( 0 );
		}
		vector< Match* >::iterator match_iter = lcb_list[ lcbI ].begin();

		// find start in each sequence
		for(; match_iter != lcb_list[ lcbI ].end(); match_iter++ ){
			for( seqI = 0; seqI < seq_count; seqI++ ){
				if( cur_lcb.start[ seqI ] == NO_MATCH &&
					(*match_iter)->Start( seqI ) != NO_MATCH ){

					cur_lcb.start[ seqI ] = (*match_iter)->Start( seqI );
					if( cur_lcb.start[ seqI ] < 0 )
						cur_lcb.start[ seqI ] -= (*match_iter)->Length();
				}
			}
		}

		// find end in each sequence
		match_iter = lcb_list[ lcbI ].end();
		for(; match_iter != lcb_list[ lcbI ].begin(); match_iter-- ){
			vector< Match* >::iterator cur_iter = match_iter;
			cur_iter--;
			for( seqI = 0; seqI < seq_count; seqI++ ){
				if( cur_lcb.length[ seqI ] == 0 &&
					(*cur_iter)->Start( seqI ) != NO_MATCH ){

					cur_lcb.length[ seqI ] = (*cur_iter)->Start( seqI ) - cur_lcb.start[ seqI ];
					if( cur_lcb.start[ seqI ] < 0 )
						cur_lcb.length[ seqI ] -= (*cur_iter)->Length();
				}
			}
		}
		
		lcb_bounds.push_back( cur_lcb );
	}

	// just do a simple O(n^2) all against all bounds check
	uint lcbJ = 0;
	for( lcbI = 0; lcbI < lcb_bounds.size(); lcbI++ ){
		for( lcbJ = 0; lcbJ < lcb_bounds.size(); lcbJ++ ){
			if( lcbI == lcbJ )
				continue;
			for( seqI = 0; seqI < seq_count; seqI++ ){
				// intersection can be acceptable in some instances
				// containment is unacceptable
				// look for containment for now.
				if( lcb_bounds[ lcbI ].start[ seqI ] == 0 ||
					lcb_bounds[ lcbJ ].start[ seqI ] == 0 )
					continue;
				if( lcb_bounds[ lcbI ].start[ seqI ] < lcb_bounds[ lcbJ ].start[ seqI ] ){
					if( lcb_bounds[ lcbI ].start[ seqI ] + lcb_bounds[ lcbI ].length[ seqI ] >
						lcb_bounds[ lcbJ ].start[ seqI ] + lcb_bounds[ lcbJ ].length[ seqI ] ){
						
						// contained
						cerr << "seqI: " << seqI << '\t';
						cerr << lcb_bounds[ lcbI ].start[ seqI ] << ".." << lcb_bounds[ lcbI ].start[ seqI ] + lcb_bounds[ lcbI ].length[ seqI ];
						cerr << " and ";
						cerr << lcb_bounds[ lcbJ ].start[ seqI ] << ".." << lcb_bounds[ lcbJ ].start[ seqI ] + lcb_bounds[ lcbJ ].length[ seqI ];
						cerr << endl;

						cerr << "LCB " << lcbJ << " is contained by LCB " << lcbI;
						cerr << endl;
					}
				}
			}
...
uint RemoveLCBandCoalesce( size_t lcbI, LcbVector& adjacencies, std::vector< double >& scores, std::vector< std::pair< uint, uint > >& id_remaps, std::vector< uint >& impact_list )
{
	...
	impact_list = vector< uint >(seq_count * (2 + seq_count * 4), LCB_UNASSIGNED);
	size_t impactI = 0;
	id_remaps.clear();
	...
	for( seqI = 0; seqI < seq_count; seqI++ ){
		...
		for( seqJ = 0; seqJ < seq_count; seqJ++ ){
			...
			if( rr_adj != -1 && ( adjacencies[rr_adj].right_adjacency[seqJ] == right_adj || 
				adjacencies[rr_adj].left_adjacency[seqJ] == right_adj ) )
				cerr << "rogue ptr\n";
			if( rl_adj != -1 && ( adjacencies[rl_adj].right_adjacency[seqJ] == right_adj || 
				adjacencies[rl_adj].left_adjacency[seqJ] == right_adj ) )
				cerr << "rogue ptr 2\n";
			// update lcbI's adjacency links to point nowhere
// 04/27/06: don't do this cuz we may want to recover the structure later
...
int64 greedyBreakpointElimination_v4( vector< LCB >& adjacencies, vector< double >& scores, SimpleBreakpointScorer& bp_scorer, ostream* status_out, bool ten_percent_society = false ){
	...
	double termination_weight = total_current_lcb_weight * 0.85;	// use this if we're discarding the first 10%
	...
	vector< pair< double, size_t > > move_heap;
	for( size_t moveI = 0; moveI < move_count; ++moveI )
	{
		move_heap.push_back( make_pair( bp_scorer(moveI), moveI ) );
	}
	...
	size_t heap_end = move_heap.size();
	while( heap_end > 0 )
	{
		std::pop_heap( move_heap.begin(), move_heap.begin()+heap_end, mshc );
		heap_end--;
		pair< double, size_t > best_move = move_heap[ heap_end ];
		if( (best_move.first < 0 && !ten_percent_society) ||
			(best_move.first < 0 && total_current_lcb_weight <= termination_weight && ten_percent_society) ||
