/*******************************************************************************
 * $Id: mormems.cpp,v 1.22 2004/03/01 02:40:08 darling Exp $
...
boolean LoadSequence(gnSequence* file_sequence, DNAFileSML* file_sar, MemHash& mh, string& test_seq1, string& test_sar1, uint32 requested_mer_size){
	...
	if(!success || recreate){
		...
		file_sar->Create(*file_sequence, requested_mer_size);
...
void Clear(MimHash& mh, vector<gnSequence*>& sequences, vector<SortedMerList*>& suffixArrays){
	mh.Clear();
...
//"diarrhea/ec-japan.fas",
	...
	string mindy_seqs[] = {"locus44\\oz44-2DEC_7A.seq",
						"locus44\\oz44-2DEC_5C.seq",
						"locus44\\oz44-2DEC_6B.seq",
						"locus44\\oz44-2DEC_9E.seq",
						"locus44\\oz44-2ECOR_14.seq",
						"locus44\\oz44-2ECOR_37.seq",
						"locus44\\oz44-2ECOR_47.seq",
						"locus44\\oz44-2EDL_933.seq",
						"locus44\\oz44-2MG1655.seq",
						"locus44\\oz44-2MN_297.seq",
						"locus44\\oz44-2WI_6.seq",
						};

	string mindy_sars[] = {"locus44\\oz44-2DEC_7A.sar",
						"locus44\\oz44-2DEC_5C.sar",
						"locus44\\oz44-2DEC_6B.sar",
						"locus44\\oz44-2DEC_9E.sar",
						"locus44\\oz44-2ECOR_14.sar",
						"locus44\\oz44-2ECOR_37.sar",
						"locus44\\oz44-2ECOR_47.sar",
						"locus44\\oz44-2EDL_933.sar",
						"locus44\\oz44-2MG1655.sar",
						"locus44\\oz44-2MN_297.sar",
						"locus44\\oz44-2WI_6.sar",
						};
		...
		switch(menu_choice[0]){
			case '1':	//align mel's sequences
				...
				for(uint32 i=0; i < seq_count; i++){
					suffixArrays.push_back(new DNAFileSML());
					sequences.push_back(new gnSequence());
					if(!LoadSequence(sequences[i], (DNAFileSML*) suffixArrays[i], mh, mindy_seqs[i], mindy_sars[i], requested_mer_size))
						return -1;
			...
			case '2':
				...
				for(uint32 i=0; i < seq_count; i++){
					suffixArrays.push_back(new DNAFileSML());
					sequences.push_back(new gnSequence());
					if(!LoadSequence(sequences[i], (DNAFileSML*) suffixArrays[i], mh, chlamydia_seqs[i], chlamydia_sars[i], requested_mer_size))
						return -1;
					ms.AddSequence(suffixArrays[i], sequences[i]);
				}
			...
			case '3':
				...
				for(uint32 i=0; i < seq_count; i++){
					suffixArrays.push_back(new DNAFileSML());
					sequences.push_back(new gnSequence());
					if(!LoadSequence(sequences[i], (DNAFileSML*) suffixArrays[i], mh, diarrhea_seqs[i], diarrhea_sars[i], requested_mer_size))
						return -1;
					ms.AddSequence(suffixArrays[i], sequences[i]);
				}
				ms.SubsequenceIdentityScore();
			...
			case '4':	//choose your own seq files
				...
				for(uint32 i=0; i < seq_count; i++){
					...
					cin >> filename;
					cout << "Enter the name of the sorted mer list file to load or create:\n" ;
					cin >> sar_filename;
					sequences.push_back(new gnSequence());
					suffixArrays.push_back(new DNAFileSML());
					if(!LoadSequence(sequences[sequences.size() - 1], (DNAFileSML*) suffixArrays[suffixArrays.size() - 1], mh, filename, sar_filename, requested_mer_size))
						return -1;
					ms.AddSequence(suffixArrays[suffixArrays.size() - 1], sequences[sequences.size() - 1]);
				}
                                ms.SubsequenceIdentityScore();
                                cout << "Identity Matrix: \n";
                                ms.PrintIDMatrix(cout);
                                ...
                                cout << "Detailed List: \n";
                                ms.PrintDetailList(cout);
				dumbfile.open("shortmemout");
				ms.WriteFile( dumbfile );
				dumbfile.close();
				AlignSequences(mh, requested_mer_size, requested_gap_tolerance);
				Clear(mh, sequences, suffixArrays);
				break;
			...
			case 'a':
				...
				suffixArrays.push_back(new DNAFileSML());
				sequences.push_back(new gnSequence());
				if(!LoadSequence(sequences[0], (DNAFileSML*) suffixArrays[0], rephash, filename, sar_filename, requested_mer_size))
					return -1;
				FindReps(rephash);
				rephash.Clear();
				Clear(mh, sequences, suffixArrays);
