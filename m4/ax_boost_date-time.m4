...
if test "$ax_cv_boost_date_time" = yes; then
  ...
  [if test "x$with_boost_date_time" != "xno"; then
    ...
    axdtls=$ax_date_time_lib-s
    axbdtls=$ax_boost_date_time_lib-s
  fi])
  for ax_lib in $ax_date_time_lib $ax_boost_date_time_lib boost_date_time $axdtls $axbdtls boost_date_time-s; do
