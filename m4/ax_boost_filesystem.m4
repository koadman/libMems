...
if test "$ax_cv_boost_filesystem" = yes; then
  ...
  [if test "x$with_boost_filesystem" != "xno"; then
    ...
    axfsls=$ax_filesystem_lib-s
    axbfsls=$ax_boost_filesystem_lib-s
  fi])
  for ax_lib in $ax_filesystem_lib $ax_boost_filesystem_lib boost_filesystem $axfsls $axbfsls boost_filesystem-s; do
